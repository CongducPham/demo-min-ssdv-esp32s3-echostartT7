...
// modified from initial code as it conflicts with already defined RESET in some libraries
// C. Pham, 2025
//
//enum {
...
static void send_ack(struct min_context *self)
{
    ...
    if (ON_WIRE_SIZE(0) <= min_tx_space(self->port)) {
        on_wire_bytes(self, TMIN_ACK, self->transport_fifo.rn, &self->transport_fifo.rn, 0, 0xffU, 1U);
...
static void send_reset(struct min_context *self)
{
    ...
    if (ON_WIRE_SIZE(0) <= min_tx_space(self->port)) {
        on_wire_bytes(self, TMIN_RESET, 0, 0, 0, 0, 0);
    }
}
